    ...
    >
      {Array.from({ length: 3 }).map((_, i) => (
        <div
          key={i}
          className={cn(
            "bg-primary animate-[typing_1.5s_infinite] rounded-full",
            dotSizes[size],
          )}
          style={{
...
export function Shimmer({
  ...
  return (
    <div
      className={cn(
        "mr-1 flex items-center gap-2",
        "bg-[linear-gradient(to_right,var(--destructive)_40%,var(--foreground)_60%,var(--destructive)_80%)]",
        "bg-size-[200%_auto] bg-clip-text font-semibold text-transparent",
        active && "animate-[shimmer_10s_infinite_linear]",
      )}
    >
      {children}
    </div>
...
export function TextShimmer({
  ...
  return (
    ...
    >
      <span className="text-base font-semibold text-transparent">{text}</span>
    </div>
  );
}
...
function Loader({
  ...
  switch (variant) {
    ...
    case "text-shimmer":
      return <TextShimmer text={text} size={size} className={className} />;
    case "loading-dots":
